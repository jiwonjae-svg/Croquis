...
from key_manager import encrypt_data, decrypt_data  # Use secure key manager
...
def encrypt_data(data: dict) -> bytes:
    """Compress and encrypt data - delegated to key_manager"""
    from key_manager import encrypt_data as km_encrypt
    return km_encrypt(data)

def decrypt_data(encrypted: bytes) -> dict:
    """Decrypt and decompress data - delegated to key_manager"""
    from key_manager import decrypt_data as km_decrypt
    return km_decrypt(encrypted)
        ...
        try:
            ...
            for file in sorted(pairs_dir.glob("*.croq"), reverse=True):
                try:
                    ...
                    # Use key_manager for decryption
                    from key_manager import get_key_manager
                    fernet = get_key_manager().get_fernet()
...
class CroquisMemoDialog(QDialog):
    ...
    def save_and_close(self):
        ...
        try:
            ...
            # Use key_manager for decryption and encryption
            from key_manager import get_key_manager
            fernet = get_key_manager().get_fernet()
            decrypted = fernet.decrypt(encrypted)
            data = json.loads(decrypted.decode())
...
class MainWindow(QMainWindow):
    ...
    def on_croquis_saved(self, original: QPixmap, screenshot: QPixmap, croquis_time: int, image_filename: str, image_metadata: dict):
        ...
        # Use key_manager for encryption
        from key_manager import get_key_manager
        fernet = get_key_manager().get_fernet()
