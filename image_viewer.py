"""
Image Viewer Window Module
Handles the croquis practice image viewer with timer and screenshot functionality
"""
...
def get_app_icon():
    ...
    return resource_loader.get_icon("/icon.ico")
...
# Import from main to avoid circular dependency
# This will be cleaned up later when we extract more modules
from main import ScreenshotOverlay, CroquisSettings
...
class ImageViewerWindow(QWidget):
    ...
    def __init__(self, settings, images: List[Any], lang: str = "ko", parent=None):
        ...
        if not self.paused and hasattr(self, 'timer') and self.timer:
            if self.settings.study_mode:
                ...
                if self.remaining_time > 0:
                    ...
                    if self.remaining_time == 0:
                        self.timer.stop()
                        # Give time for display to update to 00:00 then capture screenshot
